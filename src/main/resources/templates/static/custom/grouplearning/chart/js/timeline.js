	...
	Timeline = {
		...
		change_slide: function (type) {
			...
			if (type === 'vertical'){
				...
				$(self.wrapClass + ' .timeline-list-wrap .timeline-item').height(itemSize);
			}else {
				itemSize = $(self.wrapClass + ' .timeline-list').width();
				itemSize2 = $(self.wrapClass + ' .timeline-list').height();
				totalHeight = currentItem.outerWidth() * (self.get_count());
				currentWrapper.width(totalHeight);
				$(self.wrapClass + ' .timeline-list-wrap .timeline-item').width(itemSize);
			}
