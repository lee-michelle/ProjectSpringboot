...
(function($){
	...
	var streeSelect = {
		...
		init : function(inputWarpClass,streeWarpId,Datas,selectType, changeCallBack, isParentValid){
			this.streeInitCheckbox(inputWarpClass,streeWarpId,Datas,selectType, changeCallBack, isParentValid);
		...
		streeInitCheckbox:function(inputWarpClass,streeWarpId,Datas,selectType, changeCallBack, isParentValid){
			var plugins=[];
			if (selectType) {
			...
			.jstree({
               ...
               for(var i=0;i<nodeIds.length;i++){
	        	...
	        	if (selectType) {
	        		...
	        		var selectValues=[];
	        		var ClickId=$(this).jstree().get_bottom_selected(true); // 只能选中叶子节点，所以Parent节点是无法选中的
	        		if(isParentValid && isParentValid===true){ // 设置了isParentValid=true， 就让选中节点可以为Parent的节点
	        			ClickId = $(this).jstree().get_selected(true);
	        		}
	        	    //console.log(ClickId.length);
	        	    if (ClickId.length>0) {
		        		for (var i=0;i<ClickId.length;i++) {
			        		var ClickNode = data.instance.get_node(data.selected[i]);
			        		if(isParentValid && isParentValid===true){ //isParentValid && isParentValid===true)表示让Parent节点生效
			        			selectValues.push(ClickNode.text);
			        		}else if (ClickNode.children.length == 0){ //ClickNode.children.length == 0表示当节点下有child时，不做任何处理，与选中parent节点有效冲突
...
//		        	}
	...
	//isParentValid   当isParentValid=true时，让父节点选中生效，
	function initDropTree(treeData, selectType, changeCallBack, isParentValid){
		...
		streeSelect.init($(this).parent(), '#' + treeId ,treeData, selectType, changeCallBack, isParentValid);
	}
